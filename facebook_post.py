...
def post_to_facebook(target_id, message, photo_url, token):
    try:
        if photo_url:
            url = "https://graph.facebook.com/v19.0/" + target_id + "/photos"
            data = {"url": photo_url, "caption": message, "access_token": token}
        else:
...
def send_telegram(message):
    try:
        ...
        requests.post(url, data={"chat_id": str(CHAT_ID), "text": message, "parse_mode": "HTML"})
    except Exception as e:
        print("Telegram chyba: " + str(e))
